...
export const damage =
  (listed_damages: IListedDamage[], related_skill?: ESkillType) =>
...
export const fire_damage =
  (value: number, related_skill?: ESkillType) =>
  ({ target, turn_state }: IExecuteParams) => {
    ...
    turn_state.apply_effect(burning_effect(damage), target.id);
    return true;
  };

export const heal =
  (healing: number) =>
  ({ target }: IExecuteParams) => {
